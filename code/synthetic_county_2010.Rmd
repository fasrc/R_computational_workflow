...
study_dataset <- study_dataset[complete.cases(study_dataset[,c(1:3)]),]
write.csv(study_dataset,
...
```{r, cache=TRUE}
package_data <- read.csv("Study_dataset_2010_package_data.csv")

col_names <- colnames(study_dataset)[unlist(lapply(study_dataset, is.numeric))]

for (i in col_names){
  print(paste("Data in column ", i, "are identical: ", all.equal(study_dataset[[i]], package_data[[i]])))
}
```
